...
describe("GET /aoi/products", () => {
  it("should check if api/products url exist ", async () => {
    const response = await request(server).get("/api/products");
    expect(response.status).not.toBe(404);
  });

  it("GET a JSONcresponse with products", async () => {
    const response = await request(server).get("/api/products");
...
describe("PATCH /api/products/:id", () => {
  it("should return  response for a non-existing product", async () => {
    const productId = 2000;
...
describe("DELETE /api/products/:id", () => {
  ...
  it("should delte a product", async () => {
